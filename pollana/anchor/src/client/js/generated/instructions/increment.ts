...
import { POLLANA_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof POLLANA_PROGRAM_ADDRESS,
  TAccountPollana extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountPollana extends string
        ? WritableAccount<TAccountPollana>
        : TAccountPollana,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountPollana extends string = string> = {
  pollana: Address<TAccountPollana>;
};

export function getIncrementInstruction<
  TAccountPollana extends string,
  TProgramAddress extends Address = typeof POLLANA_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountPollana>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountPollana> {
  // Program address.
  const programAddress = config?.programAddress ?? POLLANA_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    pollana: { value: input.pollana ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.pollana)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountPollana>;
